...
KeyStatus keyst[6];
uint8_t HIDmask [16]  __attribute__((at(HIDmask_address_start)))  ;
...
void TaskADC(void *pdata)
{
	...
	while(1)
	{
		...
		for(i=0;i<6;i++)
		{
			if(readMask(HIDmask_address_adcinversion+i,(u8 *)&j,1)==1)
			{
				ADResults[i]=0x800 - ADResults[i];
			}
		}
	...
	while(1)
	{
		...
		//5,通道反向
		  ...
		  if (HIDReceive_Buffer[0] == 0x01) //handStyle=0;
			...
			{
				//adc反转
				writeMask(HIDmask_address_adcinversion,HIDReceive_Buffer+1,6);
